using UnityEngine;

namespace Bnfour.MuseDashMods.ColorScoreStatus.Data;

/// <summary>
/// Holds colors used to change the score text color.
/// </summary>
/// <param name="Main">Main score color. Taken straight from the reference grade sprite.</param>
/// <param name="Outline">Outline color (when applicable). Darkest color from the reference sprite,
/// has its HSV value reduced by additional 33 (in 0–100 range).</param>
/// <param name="Light">Light score color, used in Groove Coaster skin gradients.
/// Lightest color from the reference sprite.</param>
internal record Palette(Color Main, Color Outline, Color Light)
{
    /// <summary>
    /// Based on golden S sprite. Used for AP scores.
    /// </summary>
    internal static Palette AllPerfect => new(
        new(1f, 0.941f, 0f),
        new(0.67f, 0.536f, 0f),
        new(1f, 1f, 0.878f)
    );
    /// <summary>
    /// Based on silver S sprite. Used for non-AP FC scores.
    /// </summary>
    internal static Palette FullCombo => new(
        new(0.8f, 0.941f, 0.996f),
        new(0.391f, 0.429f, 0.595f),
        Color.white
    );
    /// <summary>
    /// Based on red S sprite. Used for non-FC scores.
    /// </summary>
    internal static Palette YouTried => new(
        new(1f, 0f, 0.529f),
        new(0.435f, 0.062f, 0.308f),
        new(1f, 0.702f, 0.984f)
    );

    /// <summary>
    /// Glow color for Groove Coaster text effects.
    /// </summary>
    internal Color GCGlow => Color.Lerp(Main, Light, 0.75f);

    /// <summary>
    /// Palette associated with a given combo status.
    /// </summary>
    /// <param name="status">Status to get colors for.</param>
    /// <returns>Palette representing the status.</returns>
    /// <exception cref="System.ApplicationException">Thrown on unknown/invalid enum value (to keep intellisense happy).</exception>
    internal static Palette ForStatus(ComboStatus status)
        => status switch
        {
            ComboStatus.AllPerfect => AllPerfect,
            ComboStatus.FullCombo => FullCombo,
            ComboStatus.ThereWasAnAttempt => YouTried,
            _ => throw new System.ApplicationException("Unknown combo status.")
        };
}
